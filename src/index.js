...
module.exports = class XBus {
    ...
    async getNearbyStops(location, n) {
        ...
        nearestStopIds.forEach( n => { nearestStops.push(stops[n.key])} );
        ...
        for (let s of stops) {
            ...
            var addPrediction = (p) => {
                if (p.direction) {
                    ...
                    const direction = p.direction.title.startsWith('Inbound') ? 'inbound' : 'outbound';
                    let time;
            ...
            if (Array.isArray(p_at_stop)) {
                p_at_stop.forEach(p => { addPrediction(p) });
            } else {
